#!/usr/bin/env python
"""Clean up unreachable devices from 192.168.56.0/24 network"""
...
def cleanup_unreachable_devices():
    """Remove unreachable devices from 192.168.56.0/24 network"""
    devices = NetworkDevice.objects.filter(network='192.168.56.0/24').order_by('ip_address')
    
    print(f"Checking {devices.count()} devices in 192.168.56.0/24 network...")
    print("=" * 80)
...
if __name__ == "__main__":
    print("Network Device Cleanup Script")
    print("=" * 40)
    print("This script will remove unreachable devices from the 192.168.56.0/24 network")
    print("Only your PC (192.168.56.1) should remain after cleanup")
    print()
    
    response = input("Do you want to continue? (y/N): ")
    if response.lower() in ['y', 'yes']:
        deleted_count = cleanup_unreachable_devices()
